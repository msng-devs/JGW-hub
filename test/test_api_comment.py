...
from backups.models import Board, Role, Member, Rank, Post, Major, Comment, Config
...
class CommentApiTestOK(APITestCase):
        ...
        for i in range(10):
            for j in range(depth_1_count[i]):
                Comment.objects.create(
                    ...
                    comment_update_time=now,
                    comment_delete=0,
                    post_post_id_pk=Comment.objects.get(
                        comment_id=i + 1
                    ).post_post_id_pk,
                    member_member_pk=Comment.objects.get(
                        comment_id=i + 1
                    ).member_member_pk,
    ...
    def __comment_get(self, data, page_size):
        def comment_get_data(ref_comment):
            instance = (
                Comment.objects.all()
                .order_by(("-" if ref_comment is None else "") + "comment_id")
                .filter(
                    post_post_id_pk=int(data["post_id"]),
                    comment_comment_id_ref=None
                    if ref_comment is None
                    else ref_comment.comment_id,
                )
            )
            ...
            return [
                {
                    ...
                    "comment_write_time": i.comment_write_time.strftime(
                        "%Y-%m-%dT%H:%M:%S.%f"
                    ),
                    "comment_update_time": i.comment_update_time.strftime(
                        "%Y-%m-%dT%H:%M:%S.%f"
                    ),
        ...
        total_count = (
            Comment.objects.all()
            .order_by("-comment_id")
            .filter(post_post_id_pk=int(data["post_id"]), comment_comment_id_ref=None)
            .count()
        )
        return {
            "count": 3,
    ...
    def test_comment_post(self):
        ...
        response: Response = self.client.post(
            self.url, data=data, content_type="application/json", **header_data
        )
        ...
        responses_data = {
            ...
            "comment_write_time": comment_instance.comment_write_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
            "comment_update_time": comment_instance.comment_update_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
    ...
    def test_comment_patch(self):
        ...
        data = {"comment_content": "수정된 사항입니다."}
        ...
        response: Response = self.client.patch(
            f"{self.url}{comment_instance.comment_id}/", data=data, **header_data
        )
        # print(response.content)
