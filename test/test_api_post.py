...
from backups.models import Board, Role, Member, Rank, Post, Major, Config

from django.utils.crypto import get_random_string

from backups.views import post_get_all_query
...
class PostApiTestOK(APITestCase):
    ...
    def __get_responses_data_pagenation(
        self, instance, query_parameters, url, total_count
    ):
        next = previous = None
        page_size = 10
        if "page" in query_parameters:
            ...
            if page != 1:
                previous = (
                    "http://testserver/hub/api/v1/post/"
                    + ("list/" if url else "")
                    + "?"
                    + "&".join(
                        [
                            f'{i[0]}={i[1] - 1 if i[0] == "page" else i[1]}'
                            for i in query
                        ]
                    )
                )
    ...
    def __get_response_data(self, instance):
        responses_data = {
            ...
            "post_write_time": instance.post_write_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
            "post_update_time": instance.post_update_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
            "board_boadr_id_pk": {
    ...
    def test_post_get_all_page(self):
        ...
        return_data = self.__get_responses_data_pagenation(
            instance, query_parameters, 1, total_count
        )
    ...
    def test_post_get_all_uid(self):
        ...
        return_data = self.__get_responses_data_pagenation(
            instance, query_parameters, 1, len_all_queryset
        )
    ...
    def test_post_post_no_img(self):
        ...
        now = datetime.datetime.now()
        board_instance = Board.objects.all()[
            random.randint(0, Board.objects.count() - 1)
        ]
        member_instance = Member.objects.all()[
            random.randint(0, Member.objects.count() - 1)
        ]
        ...
        responses_data = {
            ...
            "post_write_time": post_instance.post_write_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
            "post_update_time": post_instance.post_update_time.strftime(
                "%Y-%m-%dT%H:%M:%S.%f"
            ),
            "board_boadr_id_pk": {
    ...
    def test_post_patch_by_id(self):
        ...
        respons: Response = self.client.patch(
            f"{self.url}{key}/", data=patch_data, **header_data
        )
