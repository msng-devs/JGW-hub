...
from backups.models import Board, Post
from backups.serializers import (
    PostWriteSerializer,
    PostGetSerializer,
    PostPatchSerializer,
)
from backups.custom_pagination import (
...
def post_get_all_query(
    query_params: dict, queryset: django.db.models.query.QuerySet
    ...
    if "start_date" in query_params:
        start_date = datetime.datetime.strptime(
            query_params["start_date"], constant.TIME_QUERY
        )
    if "end_date" in query_params:
        end_date = datetime.datetime.strptime(
            query_params["end_date"], constant.TIME_QUERY
        )
    ...
    if "writer_name" in query_params:
        queryset = queryset.filter(
            member_member_pk__member_nm__contains=query_params["writer_name"]
        )
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if (
            user_role_id >= admin_role_checked
            or user_role_id
            >= instance.board_boadr_id_pk.role_role_pk_read_level.role_pk
        ):
            ...
            logger.debug(
                f"{user_uid} Post data get retrieve\tkey: {key}\ttitle: {name}"
            )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if (
            user_uid == instance.member_member_pk.member_pk
            and user_role_id
            >= instance.board_boadr_id_pk.role_role_pk_write_level.role_pk
        ):
            ...
            if "board_boadr_id_pk" in request_data:
                ...
                board_instance = Board.objects.get(
                    board_id_pk=int(request_data["board_boadr_id_pk"])
                )
                if board_instance.role_role_pk_write_level.role_pk > user_role_id:
                    # 변경하려는 게시판의 쓰기 레벨보다 요청한 유저의 권한이 낮다면 거부
                    logger.info(
                        f"{user_uid} Post patch denied - request board not allowed"
                    )
                    responses_data = {
                        "timestamp": datetime.datetime.now().isoformat(),
                        "status": 403,
    ...
    def create(self, request, *args, **kwargs):
        ...
        if (
            user_role_id >= admin_role_pk
            or user_role_id >= board_instance.role_role_pk_write_level.role_pk
        ):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if (
            user_role_id >= admin_role_pk
            or user_uid == instance.member_member_pk.member_pk
        ):
