...
from .view_check import get_logger, request_check_admin_role, request_check
...
class SurveyViewSet(viewsets.ViewSet):
    def __init__(self, **kwargs):
        ...
        self.collection_survey = __create_collection(
            self.db, constant.SURVEY_POST_DB_NME, None
        )
        self.collection_quiz = __create_collection(self.db, constant.SURVEY_QUIZ, None)
        self.collection_answer = __create_collection(
            self.db, constant.SURVEY_ANSWER, None
        )
            ...
            try:
                ...
                for q in quizzes:
                    ...
                    if type == constant.SURVEY_TEXT_CODE:  # text, current: 0
                        ...
                        type == constant.SURVEY_SELECT_ONE_CODE
                    ):  # select one, current: 1
                        quiz_data["options"] = []
                        for i in q["options"]:
                            quiz_data["options"].append({"text": i["text"]})
                        assert len(quiz_data) > 0, "There must be at least one option."
                    elif (
                        type == constant.SURVEY_SELECT_MULTIPLE_CODE
                    ):  # select multiple, current: 2
                        quiz_data["options"] = []
                        for i in q["options"]:
                            quiz_data["options"].append({"text": i["text"]})
                        assert len(quiz_data) > 0, "There must be at least one option."
                ...
                response_data["created_time"] = response_data["created_time"].strftime(
                    constant.TIME_QUERY
                )
                response_data["modified_time"] = response_data[
                    "modified_time"
                ].strftime(constant.TIME_QUERY)
                ...
                update_log = (
                    f"{user_uid} Survey Post data created"
    ...
    def create_answer(self, request, pk):
        ...
        try:
            ...
            assert survey_data["role"] <= int(
                user_role_id
            ), "User is not a survey participant."
            ...
            assert len(answers) == len(
                quizzes_data
            ), "The number of responses must equal the number of questions."
            ...
            for idx, q in enumerate(quizzes_data):
                ...
                assert (
                    q["type"] == a["type"]
                ), f'The data format of the response is different from the question. idx" {idx}'

                answer = {"parent_quiz": q["_id"]}
                ...
                if type == constant.SURVEY_TEXT_CODE:
                    ...
                    assert (
                        len(selections) > 0
                    ), "There must be at least one option selected."
                    assert len(selections) <= len(
                        q["options"]
                    ), "An option that does not exist."
                    assert selections[-1] < len(
                        q["options"]
                    ), "An option that does not exist."
                ...
                answer_data["answers"].append(answer)
            assert len(answer_data["answers"]) == len(
                quizzes_data
            ), "Invalid response data exists."
            # print(answer_data)

            if user_uid is not None:
                self.collection_answer.delete_one(
                    {"parent_post": ObjectId(pk), "user": user_uid}
                )
            ...
            update_log = (
                f"{user_uid} Answer data created"
    ...
    def list_post(self, request):
        ...
        try:
            ...
            aggregate_dict = [
                {"$match": {"$expr": {"$and": [{"$lte": ["$role", user_role_id]}]}}},
                {"$match": {"title": {"$regex": target_title}}},
                {"$sort": {"activate": -1, "created_time": -1}},
                {"$count": "count"},
            ]
            print(
                list(self.collection_survey.aggregate(aggregate_dict)),
                user_role_id,
                target_title,
            )
            ...
            if "page_size" in request.query_params:
                ...
                request.query_params["page_size"] = int(
            ...
            aggregate_dict.append(
                {
                    "$skip": (request.query_params["page"] - 1)
                    * request.query_params["page_size"]
                }
            )
            ...
            response_data = {
                ...
                if request.query_params["page"] < max_page
                else None,
    ...
    def retrieve_post(self, request, pk):
        ...
        try:
            ...
            response_data["modified_time"] = response_data["modified_time"].strftime(
                constant.TIME_QUERY
            )
            for q in quizzes_data:
                q["_id"] = str(q["_id"])
            ...
            logger.debug(
                f'{user_uid} Survey Post data get retrieve\tkey: {pk}\ttitle: {response_data["title"]}'
            )
            return Response(response_data, status=status.HTTP_200_OK)
        except Http404 as e:
    ...
    def list_answers(self, request, pk):
        ...
        if user_role_id >= admin_role_checked:
            ...
            if (
                "analyze" in request.query_params
                and int(request.query_params["analyze"]) == 1
            ):  # 분석 요청
                logger.debug(f"{user_uid} Survey Answers list analyze request")
                try:
                    assert (
                        "answer_id" in request.query_params
                    ), "The id of the response to be analyzed is required."
                    answer_id = request.query_params["answer_id"]

                    quiz_data = list(
                        self.collection_quiz.aggregate(
                            [
                                {
                                    "$match": {
                                        "parent_post": ObjectId(pk),
                    ...
                    if quiz_type == constant.SURVEY_TEXT_CODE:
                        total_count = list(
                            self.collection_answer.aggregate(
                                [
                                    {"$unwind": "$answers"},
                                    {
                                        "$match": {
                                            "answers.parent_quiz": ObjectId(answer_id)
                                        }
                                    },
                                    {"$count": "answers"},
                                ]
                            ...
                            [
                                ...
                                },
                                {
                                    "$project": {
                                        ...
                                        "count": {
                                            "$cond": [
                                                {"$anyElementTrue": "$aaa.selected"},
                                                {"$arrayElemAt": ["$aaa.selected", 0]},
                                                0,
                                            ]
                                        },
                                    }
                    ...
                    logger.error(
                        f"get list survey answers analyze failed.\n\terror: {e}"
                    )
                    detail = {
                        "timestamp": datetime.datetime.now().isoformat(),
                        "status": 400,
                ...
                try:
                    ...
                    target_answers = (
                        self.collection_answer.find({"parent_post": ObjectId(pk)})
                        .skip((page - 1) * page_size)
                        .limit(page_size)
                    )
    ...
    def delete_post(self, request, pk):
        ...
        if user_role_id >= admin_role_checked:
            ...
            try:
                ...
                result_quiz = self.collection_quiz.delete_many(
                    {"parent_post": ObjectId(pk)}
                )
                result_answer = self.collection_answer.delete_many(
                    {"parent_post": ObjectId(pk)}
                )
                logger.debug(
                    f"{user_uid} Survey Post data deleted\tkey: {pk}\tquiz count: {result_quiz.deleted_count}"
    ...
    def patch_post(self, request, pk):
        ...
        if user_role_id >= admin_role_checked:
            ...
            try:
                ...
                after_patch["created_time"] = after_patch["created_time"].strftime(
                    constant.TIME_QUERY
                )
                after_patch["modified_time"] = after_patch["modified_time"].strftime(
                    constant.TIME_QUERY
                )
                # print(after_patch)
