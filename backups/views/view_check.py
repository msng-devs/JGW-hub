...
import datetime
from backups.models import Config, Role
...
def get_user_header(
    ...
    if user_uid is None or user_role_id is None:
        ...
        logger.info(
            f"get user information success\tuser uid: {user_uid}\tuser role: {user_role_id}"
        )
...
def get_admin_role_pk() -> Union[rest_framework.response.Response, int]:
    ...
    try:
        ...
        config_admin_role = Role.objects.get(
            role_nm="ROLE_ADMIN"
        ).role_pk  # Config.objects.get(config_nm='admin_role_pk').config_val
...
def ping_pong(request):
    # 서버 health check 용도
    return Response({"detail": "pong"}, status.HTTP_200_OK)
